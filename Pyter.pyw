...
class TextWithListbox:
    ...
    # OOP-related keywords and functions
...
def gethelp():
    showinfo('Code-editor', 'Code-editor by Lavkush')
...
def colour_words():
    ...
    highlight_word(text, 'print', "yellow")
...
def run_in_Turminal():
    ...
    os.system(f'C:\\Users\\Lavkush\\AppData\\Local\\Programs\\Python\\Python312\\Lib\\idlelib\\idle.pyw -r "{path}/temp_script.py"')
...
def docs():
    os.startfile('"C:\\Users\\Lavkush\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Python 3.12\\Python 3.12 Manuals (64-bit).lnk"')
...
def read():
    pyttsx3.speak(text=text.get(1.0, END))

def venv():
    try:
        os.system(f'virtualenv .venv')
    except:
        showinfo('Error', 'You can not install virtualenv')
...
if __name__ == "__main__":
    ...
    Help.add_command(label='Read Code', command=read)
    Help.add_command(label='Create virtualenv', command=venv)
    ...
    run = Button(root, text='Run', command=run_in_Turminal).pack()
    ...
    text = Text(frame,wrap='none', height=10000, width=1000, font='Candara 14')
